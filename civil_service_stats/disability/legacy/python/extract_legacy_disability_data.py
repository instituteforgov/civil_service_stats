...
df_disability = pd.read_excel(FILE_PATH, sheet_name=SHEET_NAME)
...
df_disability = df_disability.drop(columns=cols)

df_disability.insert(0, 'id', [uuid.uuid4() for i in range(len(df_disability))])

df_disability.columns = df_disability.columns.str.strip().str.lower().str.replace(r"\s+", "_", regex=True)

df_disability = df_disability.rename(columns={"organisation": "organisation_name"})

df_disability["organisation_name"] = df_disability["organisation_name"].str.replace(r"\s*-\s*\d{4}\s*iteration\s*", "", regex=True)
