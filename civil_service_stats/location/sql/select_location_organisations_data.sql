...
WITH cs_stats_location AS (
    SELECT *, year * 4 + 1 survey_period
    FROM civil_service.civil_service_statistics_location
),
o_vicd_vodg AS (
    SELECT
        o.id,
...
SELECT 
    cs_stats_location.id,
    cs_stats_location.organisation_name [Organisation],
    cs_stats_location.year [Year],
    cs_stats_location.quarter [Quarter],
    cs_stats_location.region [Region],
    cs_stats_location.total [Total],
    CASE o_vicd_vodg.type 
        WHEN 'Reporting total' THEN 'All employees'
        ELSE o_vicd_vodg.type
    END [Organisation type],
    CASE cs_stats_location.organisation_name
...
FROM cs_stats_location   
    LEFT JOIN o_vicd_vodg ON 
        cs_stats_location.organisation_id = o_vicd_vodg.id AND
        cs_stats_location.survey_period BETWEEN o_vicd_vodg.start_period AND o_vicd_vodg.end_period 
...
ORDER BY
    cs_stats_location.year,
    cs_stats_location.organisation_name,
    cs_stats_location.region
